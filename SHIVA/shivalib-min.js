...
cloud.options=this.options;cloud.buildLayout(cloud.d);}else{switch(prop){case'font_name':d3.selectAll('text').style('font-family',this.options.font_name);break;case'backgroundColor':if(typeof this.options.backgroundColor=='undefined'||this.options.backgroundColor=='')
...
function wordCloud(container){this.d=[];this.filterSet=[];this.container=container;this.draw=function(data){$('svg').remove();$('#cloudLoad').remove();var svg=d3.select("#"+cloud.container).append("svg").attr("id","wordCloud").attr("width",cloud.options.width).attr("height",cloud.options.height)
svg.append("g").attr('fill','none').attr('scale','(1,-1)').append('rect').attr('x',0).attr('y',0).attr('width','100%').attr('height','90%').style('fill',(typeof cloud.options.backgroundColor=="undefined"||cloud.options.backgroundColor=="")?'white':'#'+cloud.options.backgroundColor);var t=d3.select('g').selectAll("text").data(data);t.enter().append("text").attr('class','word').style("font-size",function(d){return d.size+"px";}).style("font-family",cloud.options.font_name).style("fill",function(d,i){if(typeof d.color!='undefined')
return(d.color[0]=="#")?d.color:'#'+d.color;return fill(i);}).attr("text-anchor","middle").attr("transform",function(d){return"translate("+[d.x+cloud.options.width/2,d.y+cloud.options.height/2]+")rotate("+d.rotate+")";}).text(function(d){return d.text;});t.exit().remove();d3.select('svg').append('text').attr('id','cloudTitle').text(cloud.options.title).style('font-size',cloud.options.titleFontSize+'px').attr('text-anchor','middle').attr('y',cloud.options.height-(cloud.options.titleFontSize)).attr('x',cloud.options.width/2);shivaLib.SendShivaMessage("ShivaWord=ready|"+window.name);if($('#wordCloudWordList').length==0){$('#containerDiv').append($('<a>').attr('id','cloudShowListButton').css({position:'absolute',top:'10px',left:0,width:'25px',height:'20px'}).click(function(){$('#wordCloudWordList').toggle();}).button({icons:{primary:'ui-icon-script'}}).hide());$('#containerDiv').append($('<div>',{id:'wordCloudWordList',css:{position:'absolute',top:'10px',left:'20px',height:(cloud.options.height*0.6)+"px",width:'120px',borderRadius:'8px',border:'5px solid #EEE',backgroundColor:'white',overflow:'scroll',padding:'5px',}}).hide());}
...
cloud.colorize(cloud.options.spectrum);d3.selectAll('.word').on("click",function(d){console.log(d.text+" : "+d.freq);shivaLib.SendShivaMessage("ShivaWord=click|"+window.name+"|"+d.text+"|"+d.freq);});shivaLib.SendShivaMessage("ShivaWord=ready|"+window.name);};this.buildLayout=function(words){var count=0;var l=words.length;for(var i=0;i<l;i++){count+=words[i].freq;}
var avg=count/l;var distro=(words[0].freq-avg)/(words[0].freq);var fs;if(cloud.options.scale=="logarithmic")
fs=d3.scale.log().range([5,100]);else if(cloud.options.scale=="linear")
fs=d3.scale.linear().domain([0,words[0].freq]).range([5,distro*(0.15*cloud.options.height)]);else if(cloud.options.scale=="binary")
fs=d3.scale.quantile().range([0,(cloud.options.height/(words.length/5))]);var high,low;low=(typeof cloud.options.low_threshold=='undefined'||cloud.options.low_threshold=='')?0:parseInt(cloud.options.low_threshold);high=(typeof cloud.options.high_threshold=='undefined'||cloud.options.high_threshold=='')?100000000000:parseInt(cloud.options.high_threshold);words=words.filter(function(el){return el.freq>=low&&el.freq<=high;});d3.layout.cloud().size([cloud.options.width,cloud.options.height*0.8]).words(words).rotate(function(){return~~((Math.random()*2)*cloud.options.tiltRange)*((Math.random()>0.5)?1:-1);}).font(cloud.options.font_name).fontSize(function(d){return fs(d.freq);}).on("end",function(){cloud.draw(words);}).start();};this.load=function(src,algo){if(typeof algo=="undefined")
...
cloud.d=d;cloud.buildLayout(d);});};this.filter=function(){var words=[];cloud.filterSet.length=0;$('.listEntry').filter(function(){return $(this).children('span').hasClass('ui-icon-close');}).each(function(){words.push($(this).text().split(' (')[0]);});var data=cloud.d.filter(function(el){if(words.indexOf(el.text)==-1){cloud.filterSet.push(el.text);return false;}
else
return true;});cloud.buildLayout(data);};this.colorize=function(colors){var opts=colors.split(',').slice(0,-1);if(opts.length==1)
